...
この調査では、こうした問題がプロンプト、言語、文書種別、モデルのどこから生じるのかを切り分けた。そのうえで、人間が流れを追って読める文書を作るスキル `hachi-readable-writing` を作成し、GPT-5.6 Sol、GPT-5.6 Terra、GPT-5.6 Lunaで導入前後を比較した。

結論から言えば、スキルによってSolは平均84.2点から88.9点へ、Lunaは88.4点から90.4点へ改善した。最初のTerra試験は89.8点から89.1点へ下がり、同じ補正をそのまま移植できないことが分かった。そこでTerra専用の保存判定と局所修正を追加したところ、92.5点まで改善した。英語、技術文書、日常文書では基準を上回ったが、プレゼンは基準より0.7点低く、短くしすぎる課題が残った。
...
すべての生成はCodex CLI 0.144.1を使用し、推論設定を `high` に統一した。文書ごとに独立した一時セッションを作り、ユーザー設定、プロジェクト規則、別文書の会話履歴を読み込ませなかった。スキルなしとスキルありでSol、Terra、Lunaが各60文書を生成したため、評価対象は合計360文書となる。
...
Solは、一般的な能力指標やタスク指標では高い性能を示すAIエージェントです。ただし、その強さが人間向け文書の読みやすさを自動的に保証するわけではありません。今回の結果からは、通常の指示だけでは、人間が読む文書を作るための条件が不足していたと考えるのが妥当です。Solは網羅性、直接性、検証しやすさを優先する一方で、読者が説明の流れを連続して理解するための配慮が足りませんでした。この用途では、追加のライティング指示なしに使えるほど良い結果とはいえません。
...
Terraのスキルなし平均は89.8点で、Solより5.6点、Lunaより1.4点高かった。特に技術文書、とりわけ日本語で、与えられた事実を細かい節へ分解せず、設計や手順の主線を保った。日常文書は89.8点でSolとほぼ同じ、プレゼンは88.5点でSolを上回ったが、発表時間への適合にはなお注意が必要だった。

最初のスキルはTerraにそのまま移植できず、平均89.1点、改善8件、同点11件、悪化41件となった。そこで、良い草稿を保つ判定、言語と入力外断定の検査、入力に明記された優先順位の尊重、コード中心文書の範囲制御を追加した。修正後は92.5点となり、改善25件、同点21件、悪化14件まで回復した。残る弱点は、プレゼンを発表時間より短くしすぎることである。
...
日本語では、名詞句や英単語を並べるだけにせず、誰が何をなぜ行うかを文として示す。英語では、導入、本文、まとめ、チェックリストに同じ説明を重ねない。Solでは過剰な網羅と細分化、Lunaでは作業報告と完成文書の混同、Terraでは良い草稿の保存、言語不一致、入力外の断定、コードの実装化、入力の優先順位違反を重点的に確認する。
...
| Terra・全60文書 | 89.8 | **92.5** | +2.7 |
| Terra・日本語 | 92.0 | **92.3** | +0.3 |
| Terra・英語 | 87.6 | **92.7** | +5.0 |
| Terra・技術文書 | 90.5 | **91.9** | +1.4 |
| Terra・日常文書 | 89.8 | **94.4** | +4.6 |
| Terra・プレゼン | 88.5 | 87.8 | −0.7 |

Solでは37件が改善、12件が同点、11件が悪化した。Lunaでは41件が改善、8件が同点、11件が悪化した。Terra対応後は25件が改善、21件が同点、14件が悪化した。
...
悪化も残った。Lunaの認証移行プレゼン英語版は92点から87点、Solの開発ガイド英語版は50点から45点、Lunaの工事説明プレゼン英語版は91点から87点となった。Terra対応後の開発ガイドは英語版が70点から80点、日本語版が72点から80点へ改善した。一方、Terraの残る悪化は、発表時間に対して短くなりすぎたプレゼンへ集中した。

## この結果から分かったこと

第一に、AI文章の問題は言語だけでは決まらない。モデル、言語、文書種別の組み合わせで現れ方が変わる。日本語の技術文書では細切れ化、英語のプレゼンでは長文化、Lunaでは作業報告の混入、Terraでは別モデル向けの補正による過剰修正というように、同じ「読みにくさ」でも原因が異なる。

第二に、文章を人間らしくするために、箇条書きを一律に禁止したり、感情語を足したりする必要はない。重要なのは、文書が答える問いを一つに定め、段落間の因果や時間関係を言葉でつなぎ、媒体に合う量へ収めることである。

第三に、良い草稿を変更しない判断が必要である。スキルはSolとLunaの平均点を上げたが、Terraでは下げ、Lunaのプレゼンでも逆効果が出た。今後は、草稿がすでに主線、分量、言語、形式の条件を満たす場合には、モデルごとの閾値を使い、全面的な再構成ではなく局所的な修正だけを行う仕組みを強めるべきである。
...
詳細な初期調査と参考文献は [`RESEARCH_ja.md`](RESEARCH_ja.md)、スキルなしのSol評価は [`benchmarks/NATURAL_EVALUATION.md`](benchmarks/NATURAL_EVALUATION.md)、SolとLunaの基準比較は [`benchmarks/LUNA_EVALUATION.md`](benchmarks/LUNA_EVALUATION.md)、Terraの基準比較とスキル前後比較は [`benchmarks/TERRA_EVALUATION.md`](benchmarks/TERRA_EVALUATION.md) と [`benchmarks/TERRA_SKILL_EVALUATION.md`](benchmarks/TERRA_SKILL_EVALUATION.md) を参照できる。これらは統合レポートの根拠資料として残す。

Sol、Terra、Lunaを一つの表で比較した結果は [`MODEL_COMPARISON_ja.md`](MODEL_COMPARISON_ja.md) にまとめた。
