...
結論から言えば、スキルによってSolは平均84.2点から88.9点へ、Lunaは88.4点から90.4点へ改善した。一方、Terraはスキルなし89.8点から導入後89.1点へ0.7点下がった。同じ指示がすべてのモデルにそのまま効くわけではなく、Terraでは日本語、技術文書、プレゼンの多くが悪化した。導入前に4.2点あったSolとLunaの差は、導入後には1.5点まで縮まったが、モデルごとの補正が必要であることも分かった。
...
ただし、スキルはTerraにそのまま移植できなかった。スキルあり平均は89.1点となり、改善8件、同点11件、悪化41件だった。開発ガイドの英語版・日本語版では、再構成と範囲制御が強く働き、有用な実装詳細を不完全または圧縮しすぎた案内へ変えてしまった。Terraはスキルなしの段階で一部の原則を実践しているため、Terra向けには「良い草稿を保つ」判定を強め、技術文書とプレゼンへの介入を控える必要がある。
...
| Terra・全60文書 | 89.8 | 89.1 | −0.7 |
| Terra・日本語 | 92.0 | 88.9 | −3.1 |
| Terra・英語 | 87.6 | **89.4** | +1.8 |
| Terra・技術文書 | 90.5 | 86.3 | −4.3 |
| Terra・日常文書 | 89.8 | **92.4** | +2.6 |
| Terra・プレゼン | 88.5 | 85.1 | −3.4 |

Solでは37件が改善、12件が同点、11件が悪化した。Lunaでは41件が改善、8件が同点、11件が悪化した。Terraでは8件が改善、11件が同点、41件が悪化した。
...
悪化も残った。Lunaの認証移行プレゼン英語版は92点から87点、Solの開発ガイド英語版は50点から45点、Lunaの工事説明プレゼン英語版は91点から87点となった。Terraでは開発ガイドの英語版が28点、日本語版が27点下がり、過剰修正の影響がさらに大きかった。LunaとTerraでは、すでに簡潔な文書や主線のある技術文書へスキルが介入しすぎる場合がある。
